...
export function resolveDataDirectory(env = process.env) {
  ...
  if (env.XDG_CONFIG_HOME) return path.join(path.resolve(env.XDG_CONFIG_HOME), "codex-background");
...
export function normalizeConfig(input = {}, options = {}) {
  ...
  return {
    ...
    illustrationBlur: clampNumber(
      source.illustrationBlur,
      0,
      30,
      DEFAULT_CONFIG.illustrationBlur,
    ),
...
export async function writeConfig(input, options = {}) {
