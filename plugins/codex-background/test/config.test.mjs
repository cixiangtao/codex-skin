...
test("resolveDataDirectory prefers PLUGIN_DATA", () => {
  assert.equal(
    resolveDataDirectory({ PLUGIN_DATA: "/tmp/plugin-data", HOME: "/tmp/home" }),
    "/tmp/plugin-data",
  );
});
...
test("writeConfig persists normalized JSON atomically", async () => {
  ...
  try {
    const written = await writeConfig(
      { image: "/tmp/character.webp", illustrationSize: 420, illustrationX: 74, illustrationBlur: 6, illustrationOpacity: 0.65 },
      { dataDirectory },
    );
