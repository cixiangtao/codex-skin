...
为 Codex 桌面端添加全局背景图和透明人物布景的本地外观工具。它通过可视化设置页分别管理全局背景、主面板和侧边栏，不修改、不重新签名 `ChatGPT.app`。
...
该命令会打开本地设置页，以仅回环可访问的 Chrome DevTools Protocol（CDP）端口启动 Codex，注入已配置的背景，并在后台保持所有新窗口同步。Codex 退出后，后台进程也会随之结束。

首次使用时还没有背景图片，工具会先打开设置页。选择图片、调整效果，然后点击「启动背景模式」即可。终端会持续显示设置服务、背景守护进程和 Codex CDP 的端口与 PID，并提供可直接复制的停止命令。
