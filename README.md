...
> [!IMPORTANT]
> Codex Skin 是非官方项目，目前仅支持 macOS。当前发布的是 `1.0.0-beta.0` 测试版，请使用 `beta` 标签安装和运行。
...
首次使用时还没有人物图片，工具会先打开设置页。选择图片、调整效果，然后点击「启动背景模式」即可。
...
```bash
npx codex-skin@beta                  # 打开设置页并启动背景模式
npx codex-skin@beta settings         # 仅打开设置页
npx codex-skin@beta doctor           # 检查本地运行环境
npx codex-skin@beta verify           # 验证背景是否已正确显示
npx codex-skin@beta verify --reload  # 重载 Codex 后验证恢复能力
npx codex-skin@beta show             # 输出规范化后的当前配置
npx codex-skin@beta stop             # 停止后台进程并移除已注入的布景
npx codex-skin@beta disable          # 停止布景并持久化禁用状态
npx codex-skin@beta enable           # 重新启用布景配置
```

### 使用命令行配置主面板

```bash
npx codex-skin@beta configure \
  --surface main \
...
```bash
npx codex-skin@beta configure --port 9229      # 固定使用指定端口
npx codex-skin@beta configure --auto-port      # 恢复自动选择端口
```
...
后续建议始终通过 `npx codex-skin@beta` 启动，让背景连接从 Codex 启动阶段即可用。
